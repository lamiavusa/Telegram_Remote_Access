...
class Build:
    ...
    def build(self) -> None:
        ...
        """
        subprocess.run(['pyinstaller','--onefile','--noconsole','--clean','--icon='f'{self.build_dir}/src/icon.ico',
                        '--distpath', f'{self.dist_dir}','--workpath', 
                        f'{self.build_dir}/work','--specpath', f'{self.build_dir}/spec','--upx-dir', 
                        f'{self.build_dir}/upx-3.96-win64',f'{self.build_dir}/src/main.py'], shell=True, check=True)